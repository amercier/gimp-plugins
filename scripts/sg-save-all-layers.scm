...
(define (sg-save-all-layers orig-image drawable
  ...
  (define (get-visible-layers image)
    ...
    (while (> i 0)
      (set! i (- i 1))
      (if (= (car (gimp-item-get-visible (vector-ref all-layers i))) TRUE)
  ...
  (let* (
    ...
    (if (= auto-location? FALSE)
      (set! path (string-append location "/"))
    )
    (if (= auto-location? TRUE)
      (set! path (car (strbreakup (car (gimp-image-get-filename orig-image)) (car (gimp-image-get-name orig-image)))))
    )
    (while (pair? layers)
      (if (= rename? TRUE)
        (begin
      ...
      (set! fullname (string-append path fullname "." extension))
...
(script-fu-register "sg-save-all-layers"
 ...
 SF-TOGGLE "Same location as original file" TRUE
 SF-DIRNAME  "Location" ""
